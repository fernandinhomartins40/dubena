...
export function CupomPage() {
  ...
  const columns: Column<Cupom>[] = [
    ...
    { key: 'itens', header: 'Itens', cell: (v) => v.itens.length },
    { key: 'total', header: 'Total', cell: (v) => <span className="tabular-nums">{brl(v.valor_total)}</span> },
    { key: 'emit', header: 'Emitido em', cell: (v) => fmtDt(v.emitido_em) },
    {
    ...
    {
      key: 'acoes', header: '', cell: (v) => v.situacao === 'rascunho'
        ? <div className="flex justify-end"><Button variant="secondary" size="sm" loading={emitir.isPending} onClick={() => onEmitir(v)}><Send size={15} /> Emitir</Button></div>
        : null,
    },
  ]
  ...
  return (
    <div>
      <PageHeader title="Cupons fiscais (SAT/CFe)" subtitle="Cupom fiscal eletrônico — emissão e consulta"
      ...
      <DataTable columns={columns} rows={data} loading={isLoading} rowKey={(v) => v.id}
        empty={<EmptyState icon={<Receipt />} title="Nenhum cupom" />} />
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button loading={criar.isPending} onClick={onCriar}>Criar rascunho</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
