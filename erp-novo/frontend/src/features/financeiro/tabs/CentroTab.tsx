...
export function CentroTab() {
  const { data, isLoading, error, refetch } = useCentrosCusto()
  ...
  const columns: Column<CentroCusto>[] = [
    ...
    { key: 'acoes', header: '', align: 'right', width: 'w-24', cell: (c) => <div className="flex justify-end gap-1" onClick={(e) => e.stopPropagation()}><Button variant="ghost" size="icon" aria-label="Editar registro" onClick={() => setEdit(c)}><Pencil size={16} /></Button><Button variant="ghost" size="icon" aria-label="Excluir registro" onClick={() => setDel(c)}><Trash2 size={16} /></Button></div> },
  ]
  return (
    <>
      <div className="mb-3 flex justify-end"><Button onClick={() => setEdit({ ativo: 1 })}><Plus size={16} /> Novo centro</Button></div>
      <DataTable columns={columns} rows={data} loading={isLoading} error={error} onRetry={() => { void refetch() }} rowKey={(c) => c.id} onRowClick={(c) => setEdit(c)} empty={<EmptyState icon={<Wallet />} title="Nenhum centro de custo" />} />
