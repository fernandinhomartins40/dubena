...
export function PlanoTab() {
  const { data, isLoading, error, refetch } = usePlanosConta()
  ...
  const columns: Column<PlanoConta>[] = [
    ...
    { key: 'acoes', header: '', align: 'right', width: 'w-24', cell: (p) => <div className="flex justify-end gap-1" onClick={(e) => e.stopPropagation()}><Button variant="ghost" size="icon" aria-label="Editar registro" onClick={() => setEdit(p)}><Pencil size={16} /></Button><Button variant="ghost" size="icon" aria-label="Excluir registro" onClick={() => setDel(p)}><Trash2 size={16} /></Button></div> },
  ]
  return (
    <>
      <div className="mb-3 flex justify-end"><Button onClick={() => setEdit({ pagarreceber: 'R' })}><Plus size={16} /> Novo plano</Button></div>
      <DataTable columns={columns} rows={data} loading={isLoading} error={error} onRetry={() => { void refetch() }} rowKey={(p) => p.id} onRowClick={(p) => setEdit(p)} empty={<EmptyState icon={<Wallet />} title="Nenhum plano de contas" />} />
