...
class BoletoPdfController extends AbstractPdf implements PdfContract
{
    ...
    protected function comprovanteEntrega($i)
    {
    ...
    protected function Topo($i)
    {
        ...
        $enderecoPagador = $this->boleto[$i]->getPagador()->getCidade() . '/' . $this->boleto[$i]->getPagador()->getUf() . ' - ';
        $enderecoPagador .= 'CEP: ' . $this->boleto[$i]->getPagador()->getCep();
        $this->Cell(0, $this->cell, $this->_($enderecoPagador), 'BLR', 1);
    ...
    protected function Bottom($i)
    {
        ...
        $this->Cell(120, $this->cell, $this->_($this->boleto[$i]->getPagador()->getCepCidadeUf()), 'BLR');
        $this->SetFont($this->PadraoFont, '', $this->fdes);
    ...
    public function gerarBoleto($dest = self::OUTPUT_STANDARD, $save_path = null, $comprovanteEntrega = false)
    {
        ...
        for ($i = 0; $i < $this->totalBoletos; $i++) {
            ...
            if($comprovanteEntrega)
                $this->comprovanteEntrega($i);
            $this->Topo($i)->Bottom($i)->codigoBarras($i);
        }
