    ...
    private function oauthClient($data, $user)
    {
        ...
        if (($data['usaandroid'] !== null || $data['identificadorchamada'] !== null
            ...
            if ($senha != '') {
                ...
                $oauth_client->secret = base64_encode(hash_hmac('sha256', $senha, config('integracoes.oauth_client_hmac_key'), true));
            ...
            foreach ($oauth_clients as $cli) {
                $cli->secret = base64_encode(hash_hmac('sha256', $senha, config('integracoes.oauth_client_hmac_key'), true));
                $cli->save();
            }
        }
