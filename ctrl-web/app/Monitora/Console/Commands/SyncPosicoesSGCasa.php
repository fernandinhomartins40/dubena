...
namespace App\Monitora\Console\Commands;
...
class SyncPosicoesSGCasa extends Command
{
    ...
    public function handle()
    {
        ...
        $processados = [];
        DB::connection('monitora')->transaction(function () use ($pendentes, $veiculosPorTca, &$processados) {
            $linhas = [];
            $ultimas = [];
            ...
            if (! empty($linhas)) {
                ...
                foreach (array_chunk($linhas, 200) as $bloco) {
                    DB::connection('monitora')->table('positions')->insert($bloco);
                }
            }
    ...
    private function mapaVeiculosPorTca(array $idsTca)
    {
        ...
        $veiculos = DB::connection('monitora')
            ->table('veiculos')
    ...
    {
        DB::connection('monitora')->table('ultimaposicaos')
