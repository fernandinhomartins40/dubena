...
use Eduardokum\LaravelBoleto\Boleto\Render\AbstractPdf as AbstractPdf;

class BoletoService extends AbstractPdf implements PdfContract
{
    ...
    private $fcel  = 7.5; // tamanho fonte célula
    private $small = 0.1; // tamanho barra fina
    ...
    protected function instrucoes($i)
    {
        ...
        */
        ...
        if($this->showInstrucoes) {
            $this->Cell(0, 3, $this->_('Instruções de Impressão'), 0, 1, 'C');
            $this->Ln(2);
    ...
    protected function comprovanteEntrega($i, $comprovante)
    {
        if($comprovante){
            $this->traco('Comprovante de Entrega', 2);
            $this->Ln(2);
            ...
            $this->SetFont('', 'B', 10);
            $this->Cell(0, 6, 'Comprovante de Entrega', 'B', 1, 'R');

            $this->SetFont($this->PadraoFont, '', $this->fdes);
            $this->Cell(75, $this->desc, $this->_('Beneficiário'), 'TLR');
            $this->Cell(35, $this->desc, $this->_('Agência / Codigo do Beneficiário'), 'TR');
            $this->Cell(30, $this->desc, $this->_('Vencimento'), 'TR');
            $this->Cell(30, $this->desc, $this->_('Número do Documento'), 'TR', 1);


            $this->SetFont($this->PadraoFont, 'B', $this->fcel);

            $this->textFitCell(75, $this->cell, $this->_($this->boleto[$i]->getBeneficiario()->getNome()), 'LR', 0, 'L');
            $this->Cell(35, $this->cell, $this->_($this->boleto[$i]->getAgencia() . '/' . $this->boleto[$i]->getCedente()), 'R');
            $this->Cell(30, $this->cell, $this->_($this->boleto[$i]->getDataVencimento()->format('d/m/Y')), 'R');
            $this->Cell(30, $this->cell, $this->_($this->boleto[$i]->getNumeroDocumento()), 'R', 1, 'R');

            $this->SetFont($this->PadraoFont, '', $this->fdes);
            $this->Cell(75, $this->desc, $this->_('Pagador'), 'TLR');
            $this->Cell(35, $this->desc, $this->_('Carteira / Nosso Número'), 'TR');
            $this->Cell(30, $this->desc, $this->_('Espécie'), 'TR');
            $this->Cell(30, $this->desc, $this->_('Valor do Documento'), 'TR', 1);


            $this->SetFont($this->PadraoFont, 'B', $this->fcel);

            $this->textFitCell(75, $this->cell, $this->_($this->boleto[$i]->getPagador()->getNome()), 'LR', 0, 'L');

            $this->Cell(35, $this->cell, $this->_($this->boleto[$i]->getNossoNumeroBoleto()), 'R');
            $this->Cell(30, $this->cell, $this->_("R$"), 'R');
            ...
            $this->Cell(30, $this->desc, $this->_('Data de Processamento'), 'TR', 1);

            if(strpos($this->boleto[$i]->getLocalPagamento(), '#')>0){
                $localPagamento = explode('#', $this->boleto[$i]->getLocalPagamento());
                $this->SetFont($this->PadraoFont, 'B', $this->fcel);
                $this->Cell(140, $this->cell, $this->_($localPagamento[0]), 'LR');
                $this->Cell(30, $this->cell, $this->_($this->boleto[$i]->getDataProcessamento()->format('d/m/Y')), 'R', 1, 'R');
                $this->SetFont($this->PadraoFont, 'B', $this->fcel);
                $this->Cell(140, $this->cell, $this->_($localPagamento[1]), 'LR');
            ...
            $this->SetFont($this->PadraoFont, '', $this->fdes);
            $this->Cell(40, $this->desc, $this->_('Recebi(emos) o Bloqueto'), 'TLR');
            $this->Cell(25, $this->desc, $this->_('Data'), 'TR');
            $this->Cell(50, $this->desc, $this->_('Assinatura'), 'TR');
            $this->Cell(25, $this->desc, $this->_('Data'), 'TR');
            $this->Cell(30, $this->desc, $this->_('Entregador'), 'TR', 1);

            $this->SetFont($this->PadraoFont, 'B', $this->fcel);
            $this->Cell(40, $this->cell, $this->_(''), 'BLR');
            $this->Cell(25, $this->cell, $this->_(''), 'BR');
            $this->Cell(50, $this->cell, $this->_(''), 'BR');
            $this->Cell(25, $this->cell, $this->_(''), 'BR');
            $this->Cell(30, $this->cell, $this->_(''), 'BR', 1, 'R');

            $this->SetFont($this->PadraoFont, '', $this->fdes);
            $this->Cell(0, $this->desc, $this->_('Motivo de não entrega (para uso da empresa entregadora)'), '', 1, '');

            $this->SetFont($this->PadraoFont, 'B', 7);  
            $this->Cell(30, $this->cell, $this->_('( ) Mudou-se'));
            $this->Cell(25, $this->cell, $this->_('( ) Ausente'));
            $this->Cell(45, $this->cell, $this->_('( ) Não existe n. indicado'));
            $this->Cell(40, $this->cell, $this->_('( ) Recusado'));
            $this->Cell(30, $this->cell, $this->_('( ) Não procurado'), '', 1);

            $this->SetFont($this->PadraoFont, 'B', 7);  
            $this->Cell(30, $this->cell, $this->_('( ) Desconhecido'));
            $this->Cell(25, $this->cell, $this->_('( ) Falecido'));
            $this->Cell(45, $this->cell, $this->_('( ) Endereço insuficiente'));
            $this->Cell(40, $this->cell, $this->_('( ) Outros (anotar no verso)'), '', 1);
            $pulaLinha = 1;
        } else {
            $pulaLinha = 5;
        }

        $this->traco('Recibo do Pagador', $pulaLinha, 3);
        return $this;
    }
    ...
    protected function Topo($i)
    {
        ...
        $this->Cell(35, $this->desc, $this->_('Carteira / Nosso Número'), 'TR', 1);
        ...
        ///modificado by Jeferson Almeida
        $this->SetFont($this->PadraoFont, '', $this->fdes);
        $this->Cell(0, $this->cell, $this->_('Beneficiário Endereço / Sacador Avalista'), 'TLR', 1, "L");

        $enderecoBeneficiario = $this->boleto[$i]->getBeneficiario()->getEndereco() . ', ' .  $this->boleto[$i]->getBeneficiario()->getBairro() . ' - ';
        $enderecoBeneficiario .= $this->boleto[$i]->getBeneficiario()->getCidade() . '/' . $this->boleto[$i]->getBeneficiario()->getUf() . ' - ';
        $enderecoBeneficiario .= 'CEP: ' . $this->boleto[$i]->getBeneficiario()->getCep();
        $this->SetFont($this->PadraoFont, 'B', $this->fcel);
        $this->Cell(0, $this->cell, $this->_($enderecoBeneficiario), 'LR', 1);
        ...
        $this->Cell(0, $this->cell, $this->_($this->getRuaNumeroCepPagador($i)), 'LR', 1);
        $this->Cell(0, $this->cell, $this->_($this->getBairroCidadeUfPagador($i)), 'BLR', 1);

        $this->SetFont($this->PadraoFont, '', $this->fdes);
        $this->Cell(100, $this->desc, $this->_('Instruções'), 0, 0, 'L');
        $this->Cell(0, $this->desc, $this->_('Autenticação Mecânica'), 0, 1, 'R');
        $this->Ln(2);

        $pulaLinha = 12;

        $this->SetFont($this->PadraoFont, 'B', 7.5);
        ...
        $this->traco('Corte na linha pontilhada', $pulaLinha, 3);
    ...
    protected function Bottom($i)
    {
        ...
        if(strpos($this->boleto[$i]->getLocalPagamento(), '#')>0){
            $localPagamento = explode('#', $this->boleto[$i]->getLocalPagamento());
            $this->SetFont($this->PadraoFont, 'B', $this->fcel);
            $this->Cell(120, $this->cell, $this->_($localPagamento[0]), 'LR');
            $this->Cell(50, $this->cell, $this->_($this->boleto[$i]->getDataVencimento()->format('d/m/Y')), 'R', 1, 'R');
            $this->SetFont($this->PadraoFont, 'B', $this->fcel);
            $this->Cell(120, $this->cell, $this->_($localPagamento[1]), 'LR');
            $this->Cell(50, $this->cell, $this->_(''), 'R', 1, 'R');
        }
        else {
            $this->SetFont($this->PadraoFont, 'B', $this->fcel);
        ...
        $this->Cell(50, $this->cell, $this->_($this->boleto[$i]->getAgencia() . '/' . $this->boleto[$i]->getCedente()), 'R', 1, 'R');
        ...
        $this->Cell(50, $this->desc, $this->_('Carteira / Nosso número'), 'TR', 1);
        ...
        $this->Cell(25, $this->desc, $this->_('(x) Valor'), 'TR');
        ...
        $this->Cell(0, $this->cell, $this->_($this->boleto[$i]->getPagador()->getNomeDocumento()), 'LR', 1);
        $this->Cell(0, $this->cell, $this->_($this->getRuaNumeroCepPagador($i), ' -'), 'LR', 1);
        $this->Cell(120, $this->cell, $this->_($this->getBairroCidadeUfPagador($i)), 'BLR');
        $this->SetFont($this->PadraoFont, '', $this->fdes);
        $this->Cell(50, $this->cell, $this->_('Cód. Baixa'), 'BR', 1);
    ...
    private function getBairroCidadeUfPagador($i)
    {
        return $this->boleto[$i]->getPagador()->getBairro() . ' - ' . $this->boleto[$i]->getPagador()->getCidade() . '/' . $this->boleto[$i]->getPagador()->getUf();
    }

    private function getRuaNumeroCepPagador($i)
    {
        return trim($this->boleto[$i]->getPagador()->getEndereco() . '. Cep: ' . $this->boleto[$i]->getPagador()->getCep());
    }
     ...
     * @param integer $ln2
     */
    protected function traco($texto, $ln = null, $ln2 = null, $textBottom = false)
    {
        if ($ln == 1 || $ln) {
            $this->Ln($ln);
        }
    ...
    protected function codigoBarras($i)
    {
        ...
        $this->i25($this->GetX(), $this->GetY() - 15, $this->boleto[$i]->getCodigoBarras(), 0.8, 13);
        $this->traco('Corte na linha pontilhada', 5, 0, false);
    }
     ...
     * @throws \Exception
     */
    public function gerarBoleto($dest = self::OUTPUT_STANDARD, $save_path = null, $comprovanteEntrega = false)
    {
        ...
        for ($i = 0; $i < $this->totalBoletos; $i++) {
            ...
            $this->AddPage();
            $this->instrucoes($i)->comprovanteEntrega($i, $comprovanteEntrega)->Topo($i)->Bottom($i)->codigoBarras($i);
        }
