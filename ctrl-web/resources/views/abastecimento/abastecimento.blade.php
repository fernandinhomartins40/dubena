                ...
                <div class="box-header">
                    <div class="row">
                        <div class="col-md-12">
                            <div class="col-md-6" style="margin-bottom:1%">
                                @can('create', App\Veiculoabastecimento::class)
                                    <a href="{{ URL::route('veiculoabastecimento.create') }}" class="btn btn-nw-registro">Novo Registro</a>
                                @endcan
                            </div>
                            <!--col-md-6-->
                        </div>
                    ...
                    <div class="panel panel-default">
                        <div class="panel-heading">
                            <h3 class="box-title">Abastecimentos</h3>
                        </div>
                        <!-- /.box-header -->
                            ...
                            <div class="col-md-12">
                                <table id="tblCadastro" class="dataTable table table-bordered table-hover table-condensed">
                                    ...
                                    <tbody id="agencias-list" name="agencias-list">
                                        @foreach ($abastecimento as $abs)
                                        <tr id="abastecimento{{$abs->id}}">
                                            <td>{{$abs->id}}</td>
                                            <td>{{$abs->data}}</td>
                                            <td>{{$abs->placa}}</td>
                                            <td>{{$abs->nome}}</td>
                                            <td>{{$abs->kmanterior}}</td>
                                            <td>{{$abs->kmatual}}</td>
                                            <td>{{$abs->kmrodado}}</td>
                                            <td>{{conversaoLitros($abs->totallitros)}}</td>
                                            <td>{{conversaoLitros($abs->mediaconsumo)}}</td>
                                            <td>
                                                @can('view', $abs)
                                                    <button onclick="window.location.href = '{{route('veiculoabastecimento.show',$abs->id)}}'"
                                                        class='btn btn-nw-buscas btn-xs' data-toggle='tooltip' data-trigger="hover" data-placement="bottom" title="Visualizar">
                                                            <span class="fa fa-eye fa-lg"></span>
                                                    </button>
                                                @endcan
                                                @can('delete', $abs)
                                                    <button onclick="removeRegister({{$abs}})"
...
<div id='rotaDel' class="hidden">{{url('veiculoabastecimento')}}/</div>
<!--Rota para redirecionar via ajax-->
<div id='rotaIndex' class="hidden">{{route('veiculoabastecimento.index')}}</div>
