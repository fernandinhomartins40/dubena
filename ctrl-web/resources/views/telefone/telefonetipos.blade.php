...
<div id="mainContent" class="content">
    <div id="divCadastro">
        <div class="row">
                ...
                <div class="box-header">
                    <div class="row">
                        <div class="col-md-12">
                            <div class="col-md-6" style="margin-bottom:1%">
                                @can('create', App\Telefonetipo::class)
                    ...
                    <div class="panel panel-default">
                        <div class="panel-heading">
                            <h3 class="box-title">Tipos de Telefones</h3>
                        </div><!-- /.box-header -->
                        <div class="panel-body">
                            <div class="col-md-12">
                                <table id="tblCadastro" class="table table-bordered table-hover table-condensed dataTable">
                                    ...
                                    <tbody id="-list" name="telefonetipos-list">
                                        @foreach ($telefonetipos as $telefonetipo)
                                        <tr id="telefonetipo{{$telefonetipo->id}}">
                                            <td>{{$telefonetipo->id}}</td>
                                            <td>{{$telefonetipo->descricao}}</td>
                                            <td>{{$telefonetipo->celular == 1 ? 'Sim' : 'Não'}}</td>
                                            <td>{{$telefonetipo->ativo == 1 ? 'Sim' : 'Não' }}</td>
                                            <td>
                                                @can('view', $telefonetipo)
                                                    <button onclick="viewRegister({{$telefonetipo}})" 
                                                        class='btn btn-nw-buscas btn-xs' data-toggle='tooltip' data-trigger="hover" data-placement="bottom" title="Visualizar">
                                                            <span class="fa fa-eye fa-lg"></span>
                                                    </button>
                                                @endcan
                                                @can('update', $telefonetipo)
                                                    <button onclick="editRegister({{$telefonetipo}})" 
                                                        class='btn btn-nw-geral btn-xs' data-toggle='tooltip' data-trigger="hover" data-placement="bottom" title="Editar">
                                                            <span class="fa fa-pencil-square-o fa-lg"></span>
                                                    </button>
                                                @endcan
                                                @can('delete', $telefonetipo)
                                                    <button onclick="removeRegister({{$telefonetipo}})"
                        ...
                        <div class="modal-body">
                            <div class="box-body">
                                ...
                                <div class="form-group crud_space col-sm-12">
                                    ...
                                    <label for="celular" class="col-sm-1 control-label input-sm required">Celular:</label>
                                    <div class="col-sm-2 checkbox">
                                        {{ Form::checkbox('celular', 1, null, ['id'=>'celular']) }}
                                    </div>
                                </div>
                            </div>
                        </div>
            ...
            <div id='rotaStore' class="hidden">{{route('telefonetipo.store')}}</div>
            <!--Rota para atualizar via ajax-->
            <div id='rotaUpdate' class="hidden">{{url('telefonetipo')}}/</div>
            <!--Rota para deletar via ajax-->
            <div id='rotaDel' class="hidden">{{url('telefonetipo')}}/</div>
            <!--Rota para redirecionar via ajax-->
            <div id='rotaIndex' class="hidden">{{route('telefonetipo.index')}}</div>
        </div><!-- /.content-wrapper -->
    </div>
    @endsection
