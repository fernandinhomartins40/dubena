...
<div id="mainContent" class="content">
    <div id="divCadastro">
        <div class="row">
                ...
                <div class="box-header">
                    <div class="row">
                        <div class="col-md-12">
                            <div class="col-md-6" style="margin-bottom:1%">
                                @can('create', App\Tipocombustivel::class)
                    ...
                    <div class="panel panel-default">
                        <div class="panel-heading">
                            <h3 class="box-title">Tipos de Combustíveis</h3>
                        </div><!-- /.box-header -->
                        <div class="panel-body">
                            <div class="col-md-12">
                                <table id="tblCadastro" class="dataTable table table-bordered table-hover table-condensed">
                                    ...
                                    <tbody id="-list" name="tipocombustivels-list">
                                        @foreach ($tipocombustivels as $tipocombustivel)
                                        <tr id="tipocombustivel{{$tipocombustivel->id}}">
                                            <td>{{$tipocombustivel->id}}</td>
                                            <td>{{$tipocombustivel->descricao}}</td>
                                            <td>{{$tipocombustivel->ativo == 1 ? 'Sim' : 'Não' }}</td>
                                            <td>
                                                @can('view', $tipocombustivel)
                                                    <button onclick="viewRegister({{$tipocombustivel}})" 
                                                        class='btn btn-nw-buscas btn-xs' data-toggle='tooltip' data-trigger="hover" data-placement="bottom" title="Visualizar">
                                                            <span class="fa fa-eye fa-lg"></span>
                                                    </button>
                                                @endcan
                                                @can('update', $tipocombustivel)
                                                    <button onclick="editRegister({{$tipocombustivel}})" 
                                                        class='btn btn-nw-geral btn-xs' data-toggle='tooltip' data-trigger="hover" data-placement="bottom" title="Editar">
                                                            <span class="fa fa-pencil-square-o fa-lg"></span>
                                                    </button>
                                                @endcan
                                                @can('delete', $tipocombustivel)
                                                    <button onclick="removeRegister({{$tipocombustivel}})"
            ...
            <div id='rotaStore' class="hidden">{{route('tipocombustivel.store')}}</div>
            <!--Rota para atualizar via ajax-->
            <div id='rotaUpdate' class="hidden">{{url('tipocombustivel')}}/</div>
            <!--Rota para deletar via ajax-->
            <div id='rotaDel' class="hidden">{{url('tipocombustivel')}}/</div>
            <!--Rota para redirecionar via ajax-->
            <div id='rotaIndex' class="hidden">{{route('tipocombustivel.index')}}</div>
        </div><!-- /.content-wrapper -->
    </div>
    @endsection
