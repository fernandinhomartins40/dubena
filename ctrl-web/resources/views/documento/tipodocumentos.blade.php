...
<div id="mainContent" class="content">
    <div id="divCadastro">
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <div class="col-md-6" style="margin-bottom:1%">
                                @can('create', App\Tipodocumento::class)
                        ...
                        <div class="panel-body">
                            <div class="col-md-12">
                                ...
                                <table id="tblCadastro" class="dataTable table table-bordered table-hover table-condensed">
                                    ...
                                    <tbody id="-list" name="tipodocumentos-list">
                                        @foreach ($tipodocumentos as $tipodocumento)
                                        <tr id="tipodocumento{{$tipodocumento->id}}">
                                            <td>{{$tipodocumento->id}}</td>
                                            <td>{{$tipodocumento->descricao}}</td>
                                            <td>{{$tipodocumento->ativo == 1 ? 'Sim' : 'Não' }}</td>
                                            <td>
                                                @can('view', $tipodocumento)
                                                    <button onclick="viewRegister({{$tipodocumento}})" 
        ...
        <div id='rotaStore' class="hidden">{{route('tipodocumento.store')}}</div>
        <!--Rota para atualizar via ajax-->
        <div id='rotaUpdate' class="hidden">{{url('tipodocumento')}}/</div>
        <!--Rota para deletar via ajax-->
        <div id='rotaDel' class="hidden">{{url('tipodocumento')}}/</div>
        <!--Rota para redirecionar via ajax-->
        <div id='rotaIndex' class="hidden">{{route('tipodocumento.index')}}</div>
        </div><!-- /.content-wrapper -->
    </div>
</div>
@endsection
