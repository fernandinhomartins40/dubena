...
<div id="mainContent" class="content">
    <div id="divCadastro">
        <div class="row">
                ...
                <div class="box-header">
                    <div class="row">
                        <div class="col-md-12">
                            <div class="col-md-6" style="margin-bottom:1%">
                                @can('create', App\Parentesco::class)
                    ...
                    <div class="panel panel-default">
                        ...
                        <div class="panel-body">
                            <div class="col-md-12">
                                <table id="tblCadastro" btnClick='false' url='' urlupdate='' class="dataTable table table-bordered table-hover table-condensed">
                                    <thead>
                                        <tr>
                                            <th>C&oacute;digo</th>
                                    ...
                                    <tbody id="-list" name="parentescos-list">
                                        @foreach ($parentescos as $parentesco)
                                        <tr id="parentesco{{$parentesco->id}}">
                                            <td>{{$parentesco->id}}</td>
                                            <td>{{$parentesco->descricao}}</td>
                                            <td>{{$parentesco->ativo == 1 ? 'Sim' : 'Não' }}</td>
                                            <td>
                                                @can('view', $parentesco)
                                                    <button onclick="viewRegister({{$parentesco}})" 
            ...
            <div id='rotaStore' class="hidden">{{route('parentesco.store')}}</div>
            <!--Rota para atualizar via ajax-->
            <div id='rotaUpdate' class="hidden">{{url('parentesco')}}/</div>
            <!--Rota para deletar via ajax-->
            <div id='rotaDel' class="hidden">{{url('parentesco')}}/</div>
            <!--Rota para redirecionar via ajax-->
            <div id='rotaIndex' class="hidden">{{route('parentesco.index')}}</div>
        </div><!-- /.content-wrapper -->
    </div>
    @endsection
