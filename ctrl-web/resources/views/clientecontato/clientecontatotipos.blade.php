...
<div id="mainContent" class="content">
    <div id="divCadastro">
        <div class="row">
                ...
                <div class="box-header">
                    <div class="row">
                        <div class="col-md-12">
                            <div class="col-md-6" style="margin-bottom:1%">
                                @can('create', App\Clientecontatotipo::class)
                    ...
                    <div class="panel panel-default">
                        <div class="panel-heading">
                            <h3 class="box-title">Tipos de Contatos</h3>
                        </div><!-- /.box-header -->
                        <div class="panel-body">
                            <div class="col-md-12">				
                                <table id="tblCadastro" btnClick='false' class="table table-bordered table-hover table-condensed dataTable">
                                    ...
                                    <tbody id="-list" name="clientecontatotipos-list">
                                        @foreach ($clientecontatotipos as $clientecontatotipo)
                                        <tr id="clientecontatotipo{{$clientecontatotipo->id}}">
                                            <td>{{$clientecontatotipo->id}}</td>
                                            <td>{{$clientecontatotipo->descricao}}</td>
                                            <td>{{$clientecontatotipo->ativo == 1 ? 'Sim' : 'Não' }}</td>
                                            <td>
                                                @can('view', $clientecontatotipo)
                                                    <button onclick="viewRegister({{$clientecontatotipo}})" 
                                                        class='btn btn-nw-buscas btn-xs' data-toggle='tooltip' data-trigger="hover" data-placement="bottom" title="Visualizar">
                                                            <span class="fa fa-eye fa-lg"></span>
                                                    </button>
                                                @endcan
                                                @can('update', $clientecontatotipo)
                                                    <button onclick="editRegister({{$clientecontatotipo}})" 
                                                        class='btn btn-nw-geral btn-xs' data-toggle='tooltip' data-trigger="hover" data-placement="bottom" title="Editar">
                                                            <span class="fa fa-pencil-square-o fa-lg"></span>
                                                    </button>
                                                @endcan
                                                @can('delete', $clientecontatotipo)
                                                    <button onclick="removeRegister({{$clientecontatotipo}})"
            ...
            <div id='rotaStore' class="hidden">{{route('clientecontatotipo.store')}}</div>
            <!--Rota para atualizar via ajax-->
            <div id='rotaUpdate' class="hidden">{{url('clientecontatotipo')}}/</div>
            <!--Rota para deletar via ajax-->
            <div id='rotaDel' class="hidden">{{url('clientecontatotipo')}}/</div>
            <!--Rota para redirecionar via ajax-->
            <div id='rotaIndex' class="hidden">{{route('clientecontatotipo.index')}}</div>
