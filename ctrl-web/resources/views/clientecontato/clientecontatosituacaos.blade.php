...
<div id="mainContent" class="content">
    <div id="divCadastro">
        <div class="row">
                ...
                <div class="box-header">
                    <div class="row">
                        <div class="col-md-12">
                            <div class="col-md-6" style="margin-bottom:1%">
                                @can('create', App\Clientecontatosituacao::class)
                    ...
                    <div class="panel panel-default">
                        <div class="panel-heading">
                            <h3 class="box-title">Situações de Contatos</h3>
                        </div><!-- /.box-header -->
                        <div class="panel-body">
                            <div class="col-md-12">
                                <table id="tblCadastro" btnClick='false' class="dataTable table table-bordered table-hover table-condensed">
                                    <thead>
                                        <tr>
                                            <th>C&oacute;digo</th>
                                    ...
                                    <tbody id="-list" name="clientecontatosituacaos-list">
                                        @foreach ($clientecontatosituacaos as $clientecontatosituacao)
                                        <tr id="clientecontatosituacao{{$clientecontatosituacao->id}}">
                                            <td>{{$clientecontatosituacao->id}}</td>
                                            <td>{{$clientecontatosituacao->descricao}}</td>
                                            <td>{{$clientecontatosituacao->ativo == 1 ? 'Sim' : 'Não' }}</td>
                                            <td>
                                                @can('view', $clientecontatosituacao)
                                                    <button onclick="viewRegister({{$clientecontatosituacao}})" 
                                                        class='btn btn-nw-buscas btn-xs' data-toggle='tooltip' data-trigger="hover" data-placement="bottom" title="Visualizar">
                                                            <span class="fa fa-eye fa-lg"></span>
                                                    </button>
                                                @endcan
                                                @can('update', $clientecontatosituacao)
                                                    <button onclick="editRegister({{$clientecontatosituacao}})" 
                                                        class='btn btn-nw-geral btn-xs' data-toggle='tooltip' data-trigger="hover" data-placement="bottom" title="Editar">
                                                            <span class="fa fa-pencil-square-o fa-lg"></span>
                                                    </button>
                                                @endcan
                                                @can('delete', $clientecontatosituacao)
                                                    <button onclick="removeRegister({{$clientecontatosituacao}})"
            ...
            <div id='rotaStore' class="hidden">{{route('clientecontatosituacao.store')}}</div>
            <!--Rota para atualizar via ajax-->
            <div id='rotaUpdate' class="hidden">{{url('clientecontatosituacao')}}/</div>
            <!--Rota para deletar via ajax-->
            <div id='rotaDel' class="hidden">{{url('clientecontatosituacao')}}/</div>
            <!--Rota para redirecionar via ajax-->
            <div id='rotaIndex' class="hidden">{{route('clientecontatosituacao.index')}}</div>
