...
<div id="mainContent" class="content">
    <div id="divCadastro">
        <div class="row">
                ...
                <div class="box-header">
                    <div class="row">
                        <div class="col-md-12">
                            <div class="col-md-6" style="margin-bottom:1%">
                                @can('create', App\Pedidomotivoatraso::class)
                    ...
                    <div class="panel panel-default">
                        <div class="panel-heading">
                            <h3 class="box-title">Motivos de Atrasos de Pedidos</h3>
                        </div><!-- /.box-header -->
                        <div class="panel-body">
                            <div class="col-md-12">
                                <table id="tblCadastro" class="dataTable table table-bordered table-hover table-condensed">
                                    ...
                                    <tbody id="pedidomotivoatrasos-list" name="pedidomotivoatrasos-list">
                                        @foreach ($pedidomotivoatraso as $pedidomotivoatraso)
                                        <tr id="pedidomotivoatraso{{$pedidomotivoatraso->id}}">
                                            <td>{{$pedidomotivoatraso->id}}</td>
                                            <td>{{$pedidomotivoatraso->descricao}}</td>
                                            <td>{{$pedidomotivoatraso->ativo == 1 ? 'Sim' : 'Não' }}</td>
                                            <td>
                                                @can('view', $pedidomotivoatraso)
                                                    <button onclick="viewRegister({{$pedidomotivoatraso}})" 
                                                        class='btn btn-nw-buscas btn-xs' data-toggle='tooltip' data-trigger="hover" data-placement="bottom" title="Visualizar">
                                                            <span class="fa fa-eye fa-lg"></span>
                                                    </button>
                                                @endcan
                                                @can('update', $pedidomotivoatraso)
                                                    <button onclick="editRegister({{$pedidomotivoatraso}})" 
                                                        class='btn btn-nw-geral btn-xs' data-toggle='tooltip' data-trigger="hover" data-placement="bottom" title="Editar">
                                                            <span class="fa fa-pencil-square-o fa-lg"></span>
                                                    </button>
                                                @endcan
                                                @can('delete', $pedidomotivoatraso)
                                                    <button onclick="removeRegister({{$pedidomotivoatraso}})"
            ...
            <div id='rotaStore' class="hidden">{{route('pedidomotivoatraso.store')}}</div>
            <!--Rota para atualizar via ajax-->
            <div id='rotaUpdate' class="hidden">{{url('pedidomotivoatraso')}}/</div>
            <!--Rota para deletar via ajax-->
            <div id='rotaDel' class="hidden">{{url('pedidomotivoatraso')}}/</div>
            <!--Rota para redirecionar via ajax-->
            <div id='rotaIndex' class="hidden">{{route('pedidomotivoatraso.index')}}</div>
        </div><!-- /.content-wrapper -->
    </div>
    @endsection
