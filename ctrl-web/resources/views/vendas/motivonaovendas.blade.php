...
<div id="mainContent" class="content">
    <div id="divCadastro">
        <div class="row">
                ...
                <div class="box-header">
                    <div class="row">
                        <div class="col-md-12">
                            <div class="col-md-6" style="margin-bottom:1%">
                                @can('create', App\Motivonaovenda::class)
                    ...
                    <div class="panel panel-default">
                        <div class="panel-heading">
                            <h3 class="box-title">Motivos de Não Vendas</h3>
                        </div><!-- /.box-header -->
                        <div class="panel-body">
                            <div class="col-md-12">
                                <table id="tblCadastro" class="dataTable table table-bordered table-hover table-condensed">
                                    ...
                                    <tbody id="-list" name="motivonaovendas-list">
                                        @foreach ($motivonaovendas as $motivonaovenda)
                                        <tr id="motivonaovenda{{$motivonaovenda->id}}">
                                            <td>{{$motivonaovenda->id}}</td>
                                            <td>{{$motivonaovenda->descricao}}</td>
                                            <td>{{$motivonaovenda->ativo == 1 ? 'Sim' : 'Não' }}</td>
                                            <td>
                                                @can('view', $motivonaovenda)
                                                    <button onclick="viewRegister({{$motivonaovenda}})" 
                                                        class='btn btn-nw-buscas btn-xs' data-toggle='tooltip' data-trigger="hover" data-placement="bottom" title="Visualizar">
                                                            <span class="fa fa-eye fa-lg"></span>
                                                    </button>
                                                @endcan
                                                @can('update', $motivonaovenda)
                                                    <button onclick="editRegister({{$motivonaovenda}})" 
                                                        class='btn btn-nw-geral btn-xs' data-toggle='tooltip' data-trigger="hover" data-placement="bottom" title="Editar">
                                                            <span class="fa fa-pencil-square-o fa-lg"></span>
                                                    </button>
                                                @endcan
                                                @can('delete', $motivonaovenda)
                                                    <button onclick="removeRegister({{$motivonaovenda}})"
            ...
            <div id='rotaStore' class="hidden">{{route('motivonaovenda.store')}}</div>
            <!--Rota para atualizar via ajax-->
            <div id='rotaUpdate' class="hidden">{{url('motivonaovenda')}}/</div>
            <!--Rota para deletar via ajax-->
            <div id='rotaDel' class="hidden">{{url('motivonaovenda')}}/</div>
            <!--Rota para redirecionar via ajax-->
            <div id='rotaIndex' class="hidden">{{route('motivonaovenda.index')}}</div>
        </div><!-- /.content-wrapper -->
    </div>
    @endsection
