...
<div id="mainContent" class="content">
    <div id="divCadastro">
        <div class="row">
                ...
                <div class="box-header">
                    <div class="row">
                        <div class="col-md-12">                       
                            <div class="col-md-6" style="margin-bottom:1%">
                                @can('create', App\Nfgrupofiscal::class)
                    ...
                    <div class="panel panel-default">
                        <div class="panel-heading">
                            <h3 class="box-title">Grupos Fiscais</h3>
                        </div><!-- /.box-header -->
                        <div class="panel-body">
                            <div class="col-md-12">
                                <table id="tblCadastro" class="table table-bordered table-hover table-condensed dataTable">
                                    ...
                                    <tbody id="gruposfiscais-list" name="gruposfiscais-list">
                                        @foreach ($gruposfiscais as $grupofiscal)
                                        <tr id="grupofiscal{{$grupofiscal->id}}">
                                            <td>{{$grupofiscal->id}}</td>
                                            <td>{{$grupofiscal->descricao}}</td>
                                            <td>{{$grupofiscal->ativo == 1 ? 'Sim' : 'Não' }}</td>
                                            <td>
                                                @can('view', $grupofiscal)
                                                    <button onclick="viewRegister({{$grupofiscal}})" 
                                                        class='btn btn-nw-buscas btn-xs' data-toggle='tooltip' data-trigger="hover" data-placement="bottom" title="Visualizar">
                                                            <span class="fa fa-eye fa-lg"></span>
                                                    </button>
                                                @endcan
                                                @can('update', $grupofiscal)
                                                    <button onclick="editRegister({{$grupofiscal}})" 
                                                        class='btn btn-nw-geral btn-xs' data-toggle='tooltip' data-trigger="hover" data-placement="bottom" title="Editar">
                                                            <span class="fa fa-pencil-square-o fa-lg"></span>
                                                    </button>
                                                @endcan
                                                @can('delete', $grupofiscal)
                                                    <button onclick="removeRegister({{$grupofiscal}})"
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <div class="box-body">
                                ...
                                <div class="form-group crud_space col-sm-12">
                                    {!! Form::label('codigo', 'Cód Grupo Fiscal:', ['class'=>'col-sm-3 control-label input-sm']) !!}
                                    <div class="col-sm-9">
                                        {!! Form::text('id',null,['class'=>'form-control input-sm', 'id'=>'id_del']) !!}
                                    </div>
                                </div>
                                <div class="form-group crud_space col-sm-12">
            ...
            <div id='rotaStore' class="hidden">{{route('grupofiscal.store')}}</div>
            <!--Rota para atualizar via ajax-->
            <div id='rotaUpdate' class="hidden">{{url('grupofiscal')}}/</div>
            <!--Rota para deletar via ajax-->
            <div id='rotaDel' class="hidden">{{url('grupofiscal')}}/</div>
            <!--Rota para redirecionar via ajax-->
            <div id='rotaIndex' class="hidden">{{route('grupofiscal.index')}}</div>
        </div><!-- /.content-wrapper -->
    </div>
    @endsection
