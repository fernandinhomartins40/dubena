...
<div id="mainContent" class="content">
    <div id="divCadastro">
        <div class="row">
                ...
                <div class="box-header">
                    <div class="row">
                        <div class="col-md-12">
                            <div class="col-md-6" style="margin-bottom:1%">
                                @can('create', App\Nfclastrib::class)
                    ...
                    <div class="panel panel-default">
                        <div class="panel-heading">
                            <h3 class="box-title">Classificação Tributária</h3>
                        </div><!-- /.box-header -->
                        <div class="panel-body">
                            <div class="col-md-12">
                                <table id="tblCadastro" url="" btnClick="false" class="dataTable table table-bordered table-hover table-condensed">
                                    ...
                                    <tbody id="nfclastrib-list" name="nfclastrib-list">
                                        @foreach ($nfclastribs as $nfclastrib)
                                        <tr id="nfclastrib{{$nfclastrib->id}}">
                                            <td class="hidden">{{$nfclastrib->id}}</td>
                                            <td>{{$nfclastrib->codigo}}</td>
                                            <td>{{$nfclastrib->descricao}}</td>
                                            <td>
                                                @can('view', $nfclastrib)
                                                <button onclick="viewRegister({{$nfclastrib}})"
                                                        class='btn btn-nw-buscas btn-xs' data-toggle='tooltip' data-trigger="hover" data-placement="bottom" title="Visualizar">
                                                    <span class="fa fa-eye fa-lg"></span>
                                                </button>
                                                @endcan
                                                @can('update', $nfclastrib)
                                                <button onclick="editRegister({{$nfclastrib}})"
                                                        class='btn btn-nw-geral btn-xs' data-toggle='tooltip' data-trigger="hover" data-placement="bottom" title="Editar">
                                                    <span class="fa fa-pencil-square-o fa-lg"></span>
                                                </button>
                                                @endcan
                                                @can('delete', $nfclastrib)
                                                <button onclick="removeRegister({{$nfclastrib}})"
            ...
            @include('nf.partials.impostos_modals', ["classtrib" => true])
            <meta name="csrf-token" content="{{ csrf_token() }}" />
            <!--Rota para um novo cadastro via ajax-->
            <div id='rotaStore' class="hidden">{{route('nfclastrib.store')}}</div>
            <!--Rota para atualizar via ajax-->
            <div id='rotaUpdate' class="hidden">{{url('nfclastrib')}}/</div>
            <!--Rota para deletar via ajax-->
            <div id='rotaDel' class="hidden">{{url('nfclastrib')}}/</div>
            <!--Rota para redirecionar via ajax-->
            <div id='rotaIndex' class="hidden">{{route('nfclastrib.index')}}</div>
