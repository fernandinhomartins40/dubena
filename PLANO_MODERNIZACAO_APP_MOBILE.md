...
| F1 — Auth real | ⏳ Próxima | **Pré-requisito no servidor:** `AppAuthController@login` hoje exige `email`+`password`; precisa aceitar **`firebase_id_token`** validado via **Firebase Admin** para emitir o Sanctum do cliente. No app, `UserService.Login()` já está pronto para enviar o ID token; `sms.tsx` será religado a ele (hoje `GetToken`/`GetClient` são stubs 501). |
