...
| ~~**A3 — Hierarquia**~~ ✅ | Árvore `unidades → departamentos → setores_org` (BelongsToTenant + RLS) + `cargos.role_id` (papel padrão) + escopo (`unidade_id/departamento_id/setor_id/herda_filhos`) em `role_user`, gravado na atribuição. UI: aba Estrutura (master-detail) na Central de Acessos. Anti-ciclo na árvore; escopo cross-tenant descartado. (Enforcement do escopo no filtro de dados é A4/ABAC.) | Médio | A2 |
